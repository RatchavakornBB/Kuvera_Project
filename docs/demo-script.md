...
| Drafting Lead (5.1 doc/email prep, 5.2 deck prep) | **Live** — real IC memo (.docx) and IC deck (.pptx) generation via python-docx/python-pptx, a cover email draft, and a NotebookLM-style source-cited summary built directly from real risk_flagger source_excerpts (no invented citations). Downloads are backend-mediated since the storage bucket is private |
| Full contradiction/hypothesis confidence-scoring engine | **Live** — flagged contradictions are now corroboration-scored via real pgvector cosine similarity against prior flags (threshold calibrated against real embeddings, not guessed), with a resolve/refute workflow and promotion into the knowledge base on resolution |
...
- **"What about the Skill governance / model-swap admin screen?"** — "That's actually live now — the
  ...
  pass-rate scoring are live too — Pending Approvals has a real on-demand eval pass-rate bar, real
  candidate output graded by a real LLM-as-judge call against a small real test set, for the 3
  agents with eval cases defined so far."
