...
## D-011 — Concierge Q&A's "lightweight RAG" is context-stuffing, not vector search (2026-07-13)
Q: the timeline calls Concierge Q&A "lightweight RAG over seeded data." At this project's data scale (a handful of documents/tasks/notes per deal), does that mean building a real pgvector embedding pipeline?
Decision: no — `agents/deal_context.py` fetches every row for one `deal_id` across all 7 child tables and concatenates it into a single text block passed directly in the prompt. No embeddings, no pgvector, no chunking/retrieval step. This is also how the deal_id scope invariant (AGENT.md Section 11 — never blend data across deals) gets enforced: the prompt physically never contains another deal's data, which is a stronger guarantee than an instruction the model could in principle ignore. Real vector search only becomes worth its complexity once a deal's data volume can't fit in context — not the case here.
Status: LOCKED for this MVP week. Revisit if/when a deal's document set grows large enough that context-stuffing stops fitting a reasonable prompt budget.

## D-012 — Chat streaming cut per timeline Section 7 cut order item 4 (2026-07-13)
Q: the timeline's endpoint spec calls for `/chat` to stream Orchestrator tokens. Concierge Q&A's answer is produced via Claude tool-use (structured `{answer, sources}` output), not free-text — token-by-token streaming of a growing field inside a not-yet-complete tool-use JSON blob is materially more complex and error-prone than streaming plain text, and risks shipping a chat feature that intermittently renders broken/truncated text.
Decision: implemented `/chat` as request/response over the WebSocket connection (client sends `{message, deal_id?}`, server computes the full answer synchronously via `run_in_threadpool`, sends one complete message back) rather than true token streaming. This is timeline Section 7's own 4th cut-order item, invoked deliberately here rather than attempted and abandoned, and logged per AGENT.md Section 11's "log the cut, tell the user" requirement rather than silently shipping a different behavior than the doc describes.
Status: LOCKED — real token streaming would need either switching Concierge Q&A off tool-use (losing structured `sources`) or incremental JSON-streaming parsing (meaningfully more engineering than this week's clock supports). Revisit only if a future phase specifically prioritizes streaming UX.

## D-013 — Auto-NDA-on-deal-create is owned by a dedicated `nda_drafter` agent, not `drafting_lead` (2026-07-20)
Q: user asked that creating a deal auto-generate an NDA .docx into the deal's Documents tab, AI-drafted (not template). The natural home looked like the Drafting Lead (document output production). But wiring it through `call_model("drafting_lead", ...)` produced a real refusal, not an NDA — the model followed drafting_lead's DB-stored governed skill, which deliberately scopes that Lead to (a) cover email, (b) .docx/.pptx assembly from existing analysis, (c) source-cited summary, and explicitly forbids generating new content from scratch. The refusal text ("Drafting Lead has exactly three functions…") got stored as the "NDA". This is the Admin & Skill Governance chokepoint working as designed, not a bug — a spec/decision gap per AGENT.md Section 9.3, surfaced to the user rather than silently coded around.
Decision: user chose to give generative NDA drafting its own agent identity, `nda_drafter` (added to `AGENT_MODELS` in agents/adapters/model_adapter.py, model claude-sonnet-5). It has no agent_configs row yet, so `call_model()` uses an empty skill_content and the detailed NDA_PROMPT (a complete, self-contained instruction) drives the draft — while still flowing through the same `call_model()` chokepoint, so an admin can attach/version a governed NDA skill for it later via the Admin UI. This keeps generative legal drafting OUT of drafting_lead (honoring that skill's deliberate boundary). The functions physically live in agents/drafting_lead.py (draft_nda/draft_nda_docx/draft_and_store_nda) but call `call_model("nda_drafter", ...)`. NDA generation is fired best-effort on a daemon thread from create_deal — it must never block or fail deal creation (like pricing_advisor's secondary status). Output labels itself a "preliminary draft for internal review" and inserts [PLACEHOLDER] markers for unknown legal values; not presented as legally binding.
Status: LOCKED — verified live end-to-end (real Claude call → 5.7KB .docx with real numbered NDA sections, zero refusal markers, real Storage + Document row, appears in Documents tab). Two considered alternatives (extend drafting_lead's skill via the governance approval loop; switch to template-fill) were declined by the user. Revisit only if NDA content quality warrants a governed nda_drafter skill.
