...
DoD:
  - [ ] Migration: `agent_configs` (agent_name unique, model_id, skill_content, updated_at),
  ...
  - [ ] `call_model()` looks up the agent's real DB-stored model_id/skill_content on every
  ...
  - [ ] Backend: GET /admin/agents, POST /admin/agents/{name}/propose, GET
        /admin/pending-approvals, POST /admin/pending-approvals/{id}/approve|reject, GET
        /admin/audit-log
  - [ ] Frontend /admin route + Sidebar nav item: Agents & Models tab, Skills tab (editor + diff
        against current value, no fake "live markdown preview" claim — plain text diff), Pending
        Approvals tab, Audit Log tab
  - [ ] Verified end to end in a real browser AND against real agent behavior: propose a
